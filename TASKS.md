...
6. **Implementation — ACTIVE.** Implementing the WP3–WP5 architecture in `mfs9697/inmech-site` on branch `feat/ncutam-wp6-infrastructure`, draft PR #114. Completed and validated batches:
   - schema/infrastructure: NCUTAM collections, shared-news `scopes`, bilingual top-level routes, global/local navigation, landing pages, shared NCUTAM news filter;
   - current governance: 17 linked leadership/Presidium member records and the approved responsibility assignments effective from 1 December 2025, rendered bilingually from structured data;
   - evergreen/activity layer: durable About and IUTAM content, generalized bilingual body utility, activity archive and bilingual detail routes, plus a populated canonical record for the General Meeting of 11 November 2025.
   Each completed batch has passed the target repository's full `npm run validate` workflow. Full membership, verified public document binaries, remaining activity/media records, original image assets and the dedicated `check:ncutam` semantic validator remain to be migrated.
...
- The news language-block logic has been generalized into a reusable bilingual-body utility while preserving backward-compatible news exports.
...
**WP6 — continue implementation.** Import the complete current membership/in-memoriam datasets with stable IDs and consistent English names, then continue with remaining durable activities/media, verified document and image assets, and NCUTAM-specific semantic validation.

### Status
**ACTIVE — WP1 through WP5 complete; WP6 implementation in progress; three implementation batches validated in draft PR #114.**
